...
4. Kenapa kita pakai `@{}` untuk URL di Thymeleaf, bukan langsung tulis path?
   Biar gak "pecah" link-nya: Sintaks @{} itu pintar karena otomatis nambahin Context Path aplikasi. Jadi kalau nanti aplikasi kita di-deploy ke server beneran atau di-hosting di sub-folder, 
   link gambar atau CSS kita bakal tetep ketemu dan gak bakal error 404.
